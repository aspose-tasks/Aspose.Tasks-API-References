...
type: docs
weight: 130
url: /ja/python-net/aspose.tasks.saving/saveoptions/
---

## SaveOptions class

これは、プロジェクトを特定の形式で保存する際にユーザーが追加オプションを指定できるようにするクラスの抽象基底クラスです<br/>            。

SaveOptions 型は次のメンバーを公開します:
## プロパティ
| 名前 | 説明 |
| :- | :- |
| save_format | この保存オプションオブジェクトが使用された場合に、ドキュメントが保存される形式を取得または設定します。 |
