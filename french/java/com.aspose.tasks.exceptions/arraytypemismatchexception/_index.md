---
title: "ArrayTypeMismatchException"
second_title: "Référence API d'Aspose.Tasks pour Java"
description: "L'exception qui est levée lorsqu'une tentative est faite de stocker un élément d'un type incorrect dans un tableau."
type: docs
weight: 18
url: /fr/java/com.aspose.tasks.exceptions/arraytypemismatchexception/
---

**Inheritance:**
java.lang.Object, java.lang.Throwable, java.lang.Exception, java.lang.RuntimeException, [com.aspose.tasks.exceptions.Exception](../../com.aspose.tasks.exceptions/exception), [com.aspose.tasks.exceptions.SystemException](../../com.aspose.tasks.exceptions/systemexception)
```
public class ArrayTypeMismatchException extends SystemException
```

L'exception qui est levée lorsqu'une tentative est faite de stocker un élément d'un type incorrect dans un tableau.
