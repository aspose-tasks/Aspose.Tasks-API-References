---
title: "WorkUnit.WorkUnit"
second_title: "Référence de l'API Aspose.Tasks for .NET"
description: "Constructeur WorkUnit. Initialise une nouvelle instance de la classe WorkUnit. Crée un nouvel objet WorkUnit avec les dates From et To spécifiées"
type: docs
weight: 10
url: /fr/net/aspose.tasks/workunit/workunit/
---
## WorkUnit constructor

Initialise une nouvelle instance de la classe [`WorkUnit`](../). Crée un nouvel objet WorkUnit avec les dates From et To spécifiées.

```csharp
public WorkUnit(DateTime from, DateTime to)
```

| Paramètre | Type | Description |
| --- | --- | --- |
| from | DateTime | Date de début des heures de travail. |
| à | DateTime | Date de fin des heures de travail. |

## Exemples

Montre comment travailler avec les informations d'unité de travail.

```csharp
var project = new Project(DataDir + "Project1.mpp");

var calendar = project.Calendars.GetByUid(1);

// obtenez les heures de travail pour une date spécifique
var workUnit = calendar.GetWorkingHours(new DateTime(2020, 4, 8, 8, 0, 0), new DateTime(2020, 4, 9, 17, 0, 0));

Console.WriteLine("From: " + workUnit.From);
Console.WriteLine("To: " + workUnit.To);
Console.WriteLine("Working hours: " + workUnit.WorkingHours);
```

### Voir aussi

* class [WorkUnit](../)
* namespace [Aspose.Tasks](../../workunit/)
* assembly [Aspose.Tasks](../../../)


