---
title: Add
second_title: Référence de l'API Aspose.Tasks pour .NET
description: Ajoute un nouveau calendrier de base à cet objet CalendarCollection et renvoie le calendrier ajouté.
type: docs
weight: 30
url: /fr/net/aspose.tasks/calendarcollection/add/
---
## Add(string) {#add}

Ajoute un nouveau calendrier de base à cet objet CalendarCollection et renvoie le calendrier ajouté.

```csharp
public Calendar Add(string name)
```

| Paramètre | Taper | La description |
| --- | --- | --- |
| name | String | Nom du calendrier. |

### Return_Value

Ajoutée[`Calendar`](../../calendar) objet.

### Exceptions

| exception | condition |
| --- | --- |
| ArgumentException | Levé lorsque le nom du calendrier est nul. |

### Voir également

* class [Calendar](../../calendar)
* class [CalendarCollection](../../calendarcollection)
* espace de noms [Aspose.Tasks](../../calendarcollection)
* Assemblée [Aspose.Tasks](../../../)

---

## Add(string, Calendar) {#add_1}

Ajoute un nouveau calendrier avec le calendrier de base spécifié à cet objet CalendarCollection et renvoie le calendrier ajouté.

```csharp
public Calendar Add(string name, Calendar baseCalendar)
```

| Paramètre | Taper | La description |
| --- | --- | --- |
| name | String | Nom spécifié. |
| baseCalendar | Calendar | Calendrier de base spécifié. |

### Return_Value

Ajoutée[`Calendar`](../../calendar) objet.

### Voir également

* class [Calendar](../../calendar)
* class [CalendarCollection](../../calendarcollection)
* espace de noms [Aspose.Tasks](../../calendarcollection)
* Assemblée [Aspose.Tasks](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Tasks.dll -->
