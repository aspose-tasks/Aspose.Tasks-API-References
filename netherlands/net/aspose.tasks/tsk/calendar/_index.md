---
title: "Tsk.Calendar"
second_title: "Aspose.Tasks for .NET API-referentie"
description: "Tsk-veld. De taakkalender"
type: docs
weight: 160
url: /nl/net/aspose.tasks/tsk/calendar/
---
## Tsk.Calendar field

De taakkalender.

```csharp
public static readonly Key<Calendar, TaskKey> Calendar;
```

## Voorbeelden

Toont hoe taakkalenders te lezen/schrijven.

```csharp
var project = new Project(DataDir + "ReadTaskCalendar.mpp");
var task = project.RootTask.Children.Add("Task1");

// Maak een kalender aan en wijs toe aan taak
var newCalendar = project.Calendars.Add("TaskCal1");
task.Set(Tsk.Calendar, newCalendar);

var collector = new ChildTasksCollector();
TaskUtils.Apply(project.RootTask, collector, 0);

// Parseer alle recursieve kinderen
foreach (var tsk in collector.Tasks)
{
    var calendar = tsk.Get(Tsk.Calendar);
    Console.WriteLine("Task calendar name: {0}", calendar == null ? "None" : calendar.Name);
}
```

### Zie ook

* struct [Key&lt;T,K&gt;](../../key-2/)
* class [Calendar](../../calendar/)
* enum [TaskKey](../../taskkey/)
* class [Tsk](../)
* namespace [Aspose.Tasks](../../tsk/)
* assembly [Aspose.Tasks](../../../)


