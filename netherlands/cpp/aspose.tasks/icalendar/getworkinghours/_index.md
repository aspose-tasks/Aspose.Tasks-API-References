---
title: "Aspose::Tasks::ICalendar::GetWorkingHours methode"
linktitle: "GetWorkingHours"
articleTitle: "GetWorkingHours"
second_title: "Aspose.Tasks voor C++"
description: "Retourneert het aantal werkuren op de opgegeven datum."
type: docs
weight: 60
url: /nl/cpp/aspose.tasks/icalendar/getworkinghours/
---

## GetWorkingHours (1 of 2) {#getworkinghours_1}

Retourneert het aantal werkuren op de opgegeven datum.

**Returns:** Working hours at the specified date.

```cpp
GetWorkingHours(System::DateTime dt)
```

| Parameter | Beschrijving |
| --- | --- |
| dt | De datum waarvoor de werktijden opgehaald moeten worden. |

---

## GetWorkingHours (2 of 2) {#getworkinghours_2}

Retourneert WorkUnit - Start, Finish en Duration van werktijden voor het opgegeven datum‑tijdinterval.

**Returns:** Instance of WorkUnit class containing Start, Finish and Duration of working hours.

```cpp
GetWorkingHours(System::DateTime start, System::DateTime finish)
```

| Parameter | Beschrijving |
| --- | --- |
| start | Startdatum van het interval. |
| einde | Einddatum van het interval. |

