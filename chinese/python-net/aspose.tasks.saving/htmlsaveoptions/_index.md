...
type: docs
weight: 20
url: /zh/python-net/aspose.tasks.saving/htmlsaveoptions/
---
...
| 名称 | 描述 |
| :- | :- |
| HtmlSaveOptions() | 初始化 [HtmlSaveOptions](/tasks/python-net/aspose.tasks.saving/htmlsaveoptions/) 类的新实例。 |
## 属性
| 名称 | 描述 |
| :- | :- |
| save_format |  |
...
| use_gradient_brush | 获取或设置指示在渲染项目布局时是否使用渐变画刷的值。 |
...
| css_style_prefix | 获取或设置 CSS 样式前缀。 |
| font_settings | 指定在渲染项目视图时使用的字体设置。 |
| reduce_footer_gap | 获取或设置一个值，指示是否必须缩小最后任务与页脚之间的间距。 |
| include_project_name_in_page_header | 获取或设置一个值，指示是否在 HTML 页面头部包含项目名称。 |
| include_project_name_in_title | 获取或设置一个值，指示是否在 HTML 标题中包含项目名称。 |
| pages | 获取或设置在渲染项目布局时要保存的页面编号列表。 |
| export_css | 获取或设置 CSS 的导出方式。 |
| export_images | 获取或设置图像的导出方式。 |
| export_fonts | 获取或设置字体的导出方式。 |
| css_saving_callback | 获取或设置用于创建存储 CSS 资源的回调函数。 |
| font_saving_callback | 获取或设置用于创建存储字体资源的回调函数。 |
| image_saving_callback | 获取或设置用于创建存储字体资源的回调函数。 |
| font_face_types | 获取或设置字体面类型。 |
| page_saving_callback | 获取或设置用户定义的回调，用于为每个渲染的页面获取输出流。 |
