...
type: docs
weight: 40
url: /zh/python-net/aspose.tasks.saving/imagesaveoptions/
---
...
| 名称 | 描述 |
| :- | :- |
| ImageSaveOptions(save_format) | 初始化一个新的 [ImageSaveOptions](/tasks/python-net/aspose.tasks.saving/imagesaveoptions/) 类实例，可用于将渲染的图像保存为 TIFF、PNG、BMP 或 JPEG 格式。 |
## 属性
| 名称 | 描述 |
| :- | :- |
| save_format |  |
...
| jpeg_quality | 获取或设置 JPEG 质量。允许的取值范围是 0..100。 |
| reduce_footer_gap | 获取或设置一个值，指示是否必须缩小最后任务与页脚之间的间距。 |
| tiff_compression | 获取或设置保存生成的图像为 TIFF 格式时使用的压缩类型。 |
| pixel_format | 获取或设置图像中每个像素的颜色数据格式。 |
| horizontal_resolution | 获取或设置水平分辨率（dpi）。 |
| vertical_resolution | 获取或设置垂直分辨率（dpi）。 |
