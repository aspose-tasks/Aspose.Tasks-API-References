...
description: Aspose.Tasks.Saving.PdfSaveOptions 班级. 允许在将项目页面呈现为 PDF 时指定其他选项
type: docs
weight: 1860
...
| 姓名 | 描述 |
| --- | --- |
| [PdfSaveOptions](pdfsaveoptions/)() | 初始化一个新的实例`PdfSaveOptions`可用于将文档保存在[`PDF`](../savefileformat/)格式。 |
...
| [DigitalSignatureDetails](../../aspose.tasks.saving/pdfsaveoptions/digitalsignaturedetails/) { get; set; } | 获取或设置数字签名详细信息。如果未设置，则不会执行任何签名。 |
| [DrawNonWorkingTime](../../aspose.tasks.saving/saveoptions/drawnonworkingtime/) { get; set; } | 获取或设置一个值，指示是否应绘制非工作时间（默认值为 TRUE）。 |
| [EncryptionDetails](../../aspose.tasks.saving/pdfsaveoptions/encryptiondetails/) { get; set; } | 获取或设置加密详细信息。如果未设置，则不会执行任何加密。 |
...
| [FontResolveCallback](../../aspose.tasks.saving/pdfsaveoptions/fontresolvecallback/) { get; set; } | 获取或设置可用于自定义解析字体的回调。 |
...
| [Pages](../../aspose.tasks.saving/pdfsaveoptions/pages/) { get; set; } | 获取或设置将项目布局保存到单独文件时要保存的页码列表。 如果此列表为空，将保存所有页面。 |
| [PageSavingCallback](../../aspose.tasks.saving/pdfsaveoptions/pagesavingcallback/) { get; set; } | 获取或设置用户定义的回调，用于为每个呈现的页面获取输出流。 适用于[`SaveToSeparateFiles`](./savetoseparatefiles/)使用选项. |
...
| [ReduceFooterGap](../../aspose.tasks.saving/pdfsaveoptions/reducefootergap/) { get; set; } | 获取或设置一个值，该值指示是否必须减少最后一个任务和页脚之间的间隙。 |
