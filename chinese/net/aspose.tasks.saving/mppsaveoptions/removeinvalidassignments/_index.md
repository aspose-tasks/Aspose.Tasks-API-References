...
description: 获取或设置一个值该值指示保存到 MPP 时是否删除无效的资源分配 MS Project 为每个任务创建一个空的资源分配将此标志设置为 true 以在保存时将其删除
type: docs
weight: 30
...
获取或设置一个值，该值指示保存到 MPP 时是否删除无效的资源分配。 MS Project 为每个任务创建一个空的资源分配。将此标志设置为 true 以在保存时将其删除。
