...
description: "MPPSaveOptions 属性。获取或设置一个值，指示在将项目保存为 MPP 格式时是否写入组数据。组数据包括 Project.TaskGroups 和 Project.ResourceGroups 集合。"
type: docs
weight: 60
url: /zh/net/aspose.tasks.saving/mppsaveoptions/writegroups/
---
## MPPSaveOptions.WriteGroups property

获取或设置一个值，指示在将项目保存为 MPP 格式时是否写入分组数据。分组数据包括 Project.TaskGroups 和 Project.ResourceGroups 集合。
