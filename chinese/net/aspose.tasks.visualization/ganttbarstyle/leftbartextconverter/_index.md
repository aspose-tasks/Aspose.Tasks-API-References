...
description: 获取或设置用户定义的转换器以获取要在任务栏左侧呈现的文本覆盖的值LeftFieldaspose.tasks.visualization/ganttbarstyle/leftfield属性. 不保留为 MPP 格式
type: docs
weight: 100
...
获取或设置用户定义的转换器以获取要在任务栏左侧呈现的文本。覆盖的值[`LeftField`](../leftfield)属性. 不保留为 MPP 格式。
