...
| 姓名 | 描述 |
| --- | --- |
| [IsElapsed](../../aspose.tasks/duration/iselapsed) { get; } | 获取一个值，指示是否经过时间单位。 确定此 Duration 实例是否已过的标志。 |
| [IsEstimated](../../aspose.tasks/duration/isestimated) { get; } | 获取一个值，指示是否估计时间单位。 确定是否估计此 Duration 实例的标志。 |
...
| 姓名 | 描述 |
| --- | --- |
| static [Parse](../../aspose.tasks/duration/parse)(Project, string) | 将指定的字符串转换为[`Duration`](../duration)struct. |
...
| override [ToString](../../aspose.tasks/duration/tostring)() | 返回此实例的字符串表示形式。 |
| static [ParseTimeSpan](../../aspose.tasks/duration/parsetimespan)(string) | 以“PT--H--M--S--”格式解析持续时间字符串。 |
