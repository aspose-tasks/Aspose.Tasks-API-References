---
title: AndAllConditionT
second_title: Aspose.Tasks for .NET API 参考
description: 对所有条件应用逻辑与 例如cond1 AND cond2 AND cond3...
type: docs
weight: 2320
url: /zh/net/aspose.tasks.util/andallcondition-1/
---
## AndAllCondition&lt;T&gt; class

对所有条件应用逻辑与。 例如:cond1 AND cond2 AND cond3...

```csharp
public class AndAllCondition<T> : ICondition<T>
```

| 范围 | 描述 |
| --- | --- |
| T | 要应用的对象类型方法接口到。 |

## 构造函数

| 姓名 | 描述 |
| --- | --- |
| [AndAllCondition](andallcondition)(List&lt;ICondition&lt;T&gt;&gt;) | 初始化[`AndAllCondition`](../andallcondition-1)类的新实例。 |

## 方法

| 姓名 | 描述 |
| --- | --- |
| [Check](../../aspose.tasks.util/andallcondition`1/check)(T) | 如果指定对象满足条件，则返回 true。 |

### 也可以看看

* interface [ICondition&lt;T&gt;](../icondition-1)
* 命名空间 [Aspose.Tasks.Util](../../aspose.tasks.util)
* 部件 [Aspose.Tasks](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Tasks.dll -->
