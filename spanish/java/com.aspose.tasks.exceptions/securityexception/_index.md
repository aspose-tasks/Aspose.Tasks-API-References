---
title: "SecurityException"
second_title: "Referencia de API de Aspose.Tasks para Java"
description: "La excepción que se lanza cuando se detecta un error de seguridad."
type: docs
weight: 52
url: /es/java/com.aspose.tasks.exceptions/securityexception/
---

**Inheritance:**
java.lang.Object, java.lang.Throwable, java.lang.Exception, java.lang.RuntimeException, [com.aspose.tasks.exceptions.Exception](../../com.aspose.tasks.exceptions/exception), [com.aspose.tasks.exceptions.SystemException](../../com.aspose.tasks.exceptions/systemexception)
```
public class SecurityException extends SystemException
```

La excepción que se lanza cuando se detecta un error de seguridad.
