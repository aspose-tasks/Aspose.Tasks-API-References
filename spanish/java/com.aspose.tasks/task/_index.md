...
type: docs
weight: 289
url: /es/java/com.aspose.tasks/task/
---
...
Se puede usar **Task** para planificar un proyecto creando tareas y asignando los recursos apropiados a ellas. Las tareas en un proyecto se organizan como una estructura de árbol jerárquico con raíz, con una tarea raíz y subárboles de tareas hijas.

Para construir un árbol de tareas se puede usar una colección especializada [TaskCollection](../../com.aspose.tasks/taskcollection) accediendo a la propiedad `Project.RootTask`([Project.getRootTask](../../com.aspose.tasks/project\#getRootTask)/[Project.setRootTask(Task)](../../com.aspose.tasks/project\#setRootTask-Task-)), por ejemplo:
...
Task task1 = project.RootTask.Children.Add(); // se agrega una tarea padre con nombre vacío
...
```
público final NullableBool getLevelingCanSplit()
```
...
```
público final Duration getLevelingDelay()
```
...
```
público final Duration getManualDuration()
```
...
```
público final Date getManualFinish()
```
...
```
público final Date getManualStart()
```
...
```
público final String getNotesRTF()
```
...
```
público final String getNotesText()
```
...
```
público final OutlineCodeCollection getOutlineCodes()
```
...
```
público final int getOutlineLevel()
```
...
```
público final String getOutlineNumber()
```
...
```
público final BigDecimal getOvertimeCost()
```
...
```
público final Duration getOvertimeWork()
```
...
```
público final Task getParentTask()
```
...
```
público final int getPercentComplete()
```
...
```
público final int getPercentWorkComplete()
```
...
```
público final int getPhysicalPercentComplete()
```
...
```
público final TaskCollection getPredecessors()
```
...
```
público final Date getPreleveledFinish()
```
...
```
público final Date getPreleveledStart()
```
...
```
público final PrimaveraTaskProperties getPrimaveraProperties()
```
...
```
público final RecurringTaskInfo getRecurringInfo()
```
...
```
público final Duration getRegularWork()
```
...
```
público final BigDecimal getRemainingCost()
```
...
```
público final Duration getRemainingDuration()
```
...
```
público final BigDecimal getRemainingOvertimeCost()
```
...
```
público final Duration getRemainingOvertimeWork()
```
...
```
público final Duration getRemainingWork()
```
...
```
público final SplitPartCollection getSplitParts()
```
...
```
público final double getStartSlack()
```
...
```
público final String getStartText()
```
...
```
público final Duration getStartVariance()
```
...
```
público final String getStatusManager()
```
...
```
público final String getSubprojectName()
```
...
```
público final TaskCollection getSuccessors()
```
...
```
público final TimephasedDataCollection getTimephasedData()
```
...
```
público final TimephasedDataCollection getTimephasedData(Date start, Date end)
```
...
```
público final TimephasedDataCollection getTimephasedData(Date start, Date end, byte timephasedType)
```
...
```
público final double getTotalSlack()
```
...
```
público final String getWBSLevel()
```
...
```
público final boolean getWarning()
```
...
### getWork() {#getWork--}
```
público final Duration getWork()
```
...
```
público final Duration getWorkVariance()
```
