---
title: Prj.ScheduleFromStart
second_title: Referencia de Aspose.Tasks para la API de .NET
description: Prj campo. Determina si se calcula el cronograma del proyecto desde la fecha de inicio.
type: docs
weight: 620
url: /es/net/aspose.tasks/prj/schedulefromstart/
---
## Prj.ScheduleFromStart field

Determina si se calcula el cronograma del proyecto desde la fecha de inicio.

```csharp
public static readonly Key<NullableBool, PrjKey> ScheduleFromStart;
```

### Ver también

* struct [Key&lt;T,K&gt;](../../key-2/)
* struct [NullableBool](../../nullablebool/)
* enum [PrjKey](../../prjkey/)
* class [Prj](../)
* espacio de nombres [Aspose.Tasks](../../prj/)
* asamblea [Aspose.Tasks](../../../)


