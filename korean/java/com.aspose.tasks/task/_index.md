...
type: docs
weight: 289
url: /ko/java/com.aspose.tasks/task/
---
...
**Task**는 하나의 원자적 작업 단위를 나타냅니다.

프로젝트를 계획하려면 **Task**를 사용하여 작업을 생성하고 적절한 리소스를 할당할 수 있습니다. 프로젝트의 작업은 루트 작업과 하위 작업 트리로 구성된 계층적 트리 구조로 조직됩니다.

작업 트리를 구축하려면 `Project.RootTask`([Project.getRootTask](../../com.aspose.tasks/project\#getRootTask)/[Project.setRootTask(Task)](../../com.aspose.tasks/project\#setRootTask-Task-)) 속성에 접근하여 특수 컬렉션 [TaskCollection](../../com.aspose/tasks/taskcollection)을 사용할 수 있습니다. 예:
...
// 새 작업 추가
Task task1 = project.RootTask.Children.Add(); // 이름이 비어 있는 상위 작업이 추가됩니다
...
project.Save("채워진 프로젝트.xml", SaveFileFormat.Mpp);
...
```
public final 날짜 getCommitmentFinish()
```
...
```
public final 날짜 getCommitmentStart()
```
...
```
public final 정수 getCommitmentType()
```
...
```
public final 날짜 getConstraintDate()
```
...
```
public final 정수 getConstraintType()
```
...
```
public final 실수 getCostVariance()
```
...
```
public final 불리언 getDisplayOnTimeline()
```
...
```
public final 바이트 getDurationFormat()
```
...
```
public final 문자열 getDurationText()
```
...
```
public final 기간 getDurationVariance()
```
...
```
public final 날짜 getEarlyFinish()
```
...
```
public final 정수 getEarnedValueMethod()
```
...
```
public final 문자열 getExternalTaskProject()
```
...
```
public final 정수 getExternalUid()
```
...
```
public final 실수 getFinishSlack()
```
