---
title: ProjectExternallyEdited
second_title: .NET API 참조용 Aspose.Tasks
description: ProjectExternallyEdited 설정 여부를 나타내는 값을 가져오거나 설정합니다.
type: docs
weight: 700
url: /ko/net/aspose.tasks/project/projectexternallyedited/
---
## Project.ProjectExternallyEdited property

ProjectExternallyEdited 설정 여부를 나타내는 값을 가져오거나 설정합니다.

```csharp
public NullableBool ProjectExternallyEdited { get; set; }
```

### 또한보십시오

* struct [NullableBool](../../nullablebool/)
* class [Project](../)
* 네임스페이스 [Aspose.Tasks](../../project/)
* 집회 [Aspose.Tasks](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Tasks.dll -->
