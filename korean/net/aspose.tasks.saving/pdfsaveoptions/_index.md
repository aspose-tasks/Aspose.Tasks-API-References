...
| 이름 | 설명 |
| --- | --- |
| [PdfSaveOptions](pdfsaveoptions/)() | 의 새 인스턴스를 초기화합니다.`PdfSaveOptions`문서를 저장하는 데 사용할 수 있는 클래스[`PDF`](../savefileformat/) 체재. |
...
| [Compliance](../../aspose.tasks.saving/pdfsaveoptions/compliance/) { get; set; } | 생성된 PDF 문서에 대해 원하는 준수 수준을 가져오거나 설정합니다. 기본값은 다음과 같습니다.Pdf15 . |
| [CustomPageSize](../../aspose.tasks.saving/saveoptions/custompagesize/) { get; set; } | 사용자 지정 페이지 크기를 포인트 단위로 가져오거나 설정합니다(1포인트 = 인치의 1/72). |
| [DefaultFontName](../../aspose.tasks.saving/pdfsaveoptions/defaultfontname/) { get; set; } | 렌더링을 위한 기본 글꼴을 가져오거나 설정합니다. |
| [DigitalSignatureDetails](../../aspose.tasks.saving/pdfsaveoptions/digitalsignaturedetails/) { get; set; } | 디지털 서명 정보를 가져오거나 설정합니다. 설정하지 않으면 서명이 수행되지 않습니다. |
...
| [FontResolveCallback](../../aspose.tasks.saving/pdfsaveoptions/fontresolvecallback/) { get; set; } | 해결된 글꼴을 사용자 지정하는 데 사용할 수 있는 콜백을 가져오거나 설정합니다. |
...
| [Pages](../../aspose.tasks.saving/pdfsaveoptions/pages/) { get; set; } | 프로젝트 레이아웃을 별도의 파일로 저장할 때 저장할 페이지 번호 목록을 가져오거나 설정합니다. 이 목록이 비어 있으면 모든 페이지가 저장됩니다. |
| [PageSavingCallback](../../aspose.tasks.saving/pdfsaveoptions/pagesavingcallback/) { get; set; } | 렌더링된 각 페이지에 대한 출력 스트림을 가져오는 데 사용되는 사용자 정의 콜백을 가져오거나 설정합니다. 적용 가능한 경우[`SaveToSeparateFiles`](./savetoseparatefiles/) 옵션이 사용되었습니다. |
...
| [ReduceFooterGap](../../aspose.tasks.saving/pdfsaveoptions/reducefootergap/) { get; set; } | 마지막 작업과 바닥글 사이의 간격을 줄여야 하는지 여부를 나타내는 값을 가져오거나 설정합니다. |
...
| [SaveToSeparateFiles](../../aspose.tasks.saving/pdfsaveoptions/savetoseparatefiles/) { get; set; } | 프로젝트 페이지를 별도의 파일로 저장할지 여부를 나타내는 값을 가져오거나 설정합니다. |
...
| [TextCompression](../../aspose.tasks.saving/pdfsaveoptions/textcompression/) { get; set; } | 이미지를 제외한 모든 콘텐츠 스트림에 사용할 압축 유형을 가져오거나 설정합니다. 기본값은Flate . |
...
| [UseProjectDefaultFont](../../aspose.tasks.saving/pdfsaveoptions/useprojectdefaultfont/) { get; set; } | 렌더링에 기본 글꼴을 사용해야 하는지 여부를 나타내는 값을 가져오거나 설정합니다. |
