---
title: RemainingOvertimeWork
second_title: Aspose.Tasks για Αναφορά API .NET
description: Οι υπόλοιπες υπερωρίες που έχουν προγραμματιστεί για την ολοκλήρωση μιας εργασίας.
type: docs
weight: 450
url: /el/net/aspose.tasks/asn/remainingovertimework/
---
## Asn.RemainingOvertimeWork field

Οι υπόλοιπες υπερωρίες που έχουν προγραμματιστεί για την ολοκλήρωση μιας εργασίας.

```csharp
public static readonly Key<Duration, AsnKey> RemainingOvertimeWork;
```

### Δείτε επίσης

* struct [Key&lt;T,K&gt;](../../key-2/)
* struct [Duration](../../duration/)
* enum [AsnKey](../../asnkey/)
* class [Asn](../)
* χώρος ονομάτων [Aspose.Tasks](../../asn/)
* συνέλευση [Aspose.Tasks](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Tasks.dll -->
