---
title: OutlineMaskCollection.Remove
second_title: Aspose.Tasks för .NET API-referens
description: OutlineMaskCollection metod. Tar bort den första förekomsten av ett specifikt objekt från den här samlingen.
type: docs
weight: 110
url: /sv/net/aspose.tasks/outlinemaskcollection/remove/
---
## OutlineMaskCollection.Remove method

Tar bort den första förekomsten av ett specifikt objekt från den här samlingen.

```csharp
public bool Remove(OutlineMask item)
```

| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| item | OutlineMask | det angivna objektet att ta bort. |

### Returvärde

true om det angivna objektet har tagits bort från den här samlingen; annars falskt.

### Se även

* class [OutlineMask](../../outlinemask/)
* class [OutlineMaskCollection](../)
* namnutrymme [Aspose.Tasks](../../outlinemaskcollection/)
* hopsättning [Aspose.Tasks](../../../)


