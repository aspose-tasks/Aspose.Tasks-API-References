...
description: "ExtendedAttributeDefinition метод. Фабричный метод, который создает определение расширенного атрибута с поиском. Имеет CalculationType, равный Lookup, и может использоваться только в ресурсах. Требуется указать fieldId и alias при вызове этого метода. Тип поля выводится из fieldId."
type: docs
weight: 10
...
Фабричный метод, который создает определение расширенного атрибута с поиском. Имеет [`CalculationType`](../calculationtype/) равный Lookup и может использоваться только в ресурсах. Требуется указать *fieldId* и *alias* при вызове этого метода. Тип поля выводится из fieldId.
...
Используйте этот пример для создания определения пользовательского поля для ресурса с поиском, а затем заполните его текстовыми значениями:
...
Фабричный метод, который создает определение расширенного атрибута с поиском. Имеет [`CalculationType`](../calculationtype/) равный Lookup и может использоваться только в ресурсах. Требуется указать *customFieldType*, *fieldId* и *alias* при вызове этого метода.
