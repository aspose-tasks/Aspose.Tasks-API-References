---
title: OutlineCodeDefinition.EnterpriseOutlineCodeAlias
second_title: Справочник по Aspose.Tasks для .NET API
description: OutlineCodeDefinition свойство. Получает или задает ссылку на другое настраиваемое поле псевдонимом которого является это определение кода схемы.
type: docs
weight: 50
url: /ru/net/aspose.tasks/outlinecodedefinition/enterpriseoutlinecodealias/
---
## OutlineCodeDefinition.EnterpriseOutlineCodeAlias property

Получает или задает ссылку на другое настраиваемое поле, псевдонимом которого является это определение кода схемы.

```csharp
public int EnterpriseOutlineCodeAlias { get; set; }
```

### Смотрите также

* class [OutlineCodeDefinition](../)
* пространство имен [Aspose.Tasks](../../outlinecodedefinition/)
* сборка [Aspose.Tasks](../../../)


