---
title: TimephasedDataCollection.Contains
second_title: Справочник по Aspose.Tasks для .NET API
description: TimephasedDataCollection метод. ОпределяетTimephasedDataCollection содержит определенное значение.
type: docs
weight: 80
url: /ru/net/aspose.tasks/timephaseddatacollection/contains/
---
## TimephasedDataCollection.Contains method

Определяет,[`TimephasedDataCollection`](../) содержит определенное значение.

```csharp
public bool Contains(TimephasedData item)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| item | TimephasedData | Объект, который нужно найти в коллекции. |

### Возвращаемое значение

верно, если*item* находится в коллекции; в противном случае ложно.

### Смотрите также

* class [TimephasedData](../../timephaseddata/)
* class [TimephasedDataCollection](../)
* пространство имен [Aspose.Tasks](../../timephaseddatacollection/)
* сборка [Aspose.Tasks](../../../)


