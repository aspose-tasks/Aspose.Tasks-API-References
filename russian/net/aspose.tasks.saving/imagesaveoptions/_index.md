...
description: Aspose.Tasks.Saving.ImageSaveOptions сорт. Позволяет указать дополнительные параметры при рендеринге страниц проекта в изображения.
type: docs
weight: 1770
url: /ru/net/aspose.tasks.saving/imagesaveoptions/
---
...
| Имя | Описание |
| --- | --- |
| [ImageSaveOptions](imagesaveoptions/)(SaveFileFormat) | Инициализирует новый экземпляр`ImageSaveOptions` класс, который можно использовать для сохранения обработанных изображений в форматах TIFF, PNG, BMP или JPEG. |
...
| [DefaultFontName](../../aspose.tasks.saving/imagesaveoptions/defaultfontname/) { get; set; } | Получает или задает шрифт по умолчанию для рендеринга. |
...
| [FontResolveCallback](../../aspose.tasks.saving/imagesaveoptions/fontresolvecallback/) { get; set; } | Получает или задает обратный вызов, который можно использовать для настройки разрешенных шрифтов. |
...
| [HorizontalResolution](../../aspose.tasks.saving/imagesaveoptions/horizontalresolution/) { get; set; } | Получает или задает разрешение по горизонтали в dpi. |
| [JpegQuality](../../aspose.tasks.saving/imagesaveoptions/jpegquality/) { get; set; } | Получает или задает качество JPEG. Допустимый диапазон значений: 0..100. |
...
| [Pages](../../aspose.tasks.saving/imagesaveoptions/pages/) { get; set; } | Получает или задает список номеров страниц для сохранения при сохранении макета проекта в отдельные файлы. Все страницы будут сохранены, если этот список пуст. |
| [PageSavingCallback](../../aspose.tasks.saving/imagesaveoptions/pagesavingcallback/) { get; set; } | Получает или задает определяемый пользователем обратный вызов, который используется для получения выходного потока для каждой отображаемой страницы. |
...
| [PixelFormat](../../aspose.tasks.saving/imagesaveoptions/pixelformat/) { get; set; } | Получает или задает формат данных цвета для каждого пикселя изображения. |
...
| [ReduceFooterGap](../../aspose.tasks.saving/imagesaveoptions/reducefootergap/) { get; set; } | Получает или задает значение, указывающее, должен ли быть уменьшен разрыв между последней задачей и нижним колонтитулом. |
...
| [TiffCompression](../../aspose.tasks.saving/imagesaveoptions/tiffcompression/) { get; set; } | Получает или задает тип сжатия, применяемый при сохранении сгенерированных изображений в формате TIFF. |
...
| [UseProjectDefaultFont](../../aspose.tasks.saving/imagesaveoptions/useprojectdefaultfont/) { get; set; } | Получает или задает значение, указывающее, должен ли использоваться шрифт по умолчанию для рендеринга. |
| [VerticalResolution](../../aspose.tasks.saving/imagesaveoptions/verticalresolution/) { get; set; } | Получает или задает вертикальное разрешение в dpi. |
