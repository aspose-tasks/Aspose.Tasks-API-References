...
description: "Свойство GanttBarStyle. Получает или задает родительский или общий стиль для пользовательского стиля задачи"
type: docs
weight: 160
...
Получает или задает родительский (или общий) стиль для пользовательского стиля, специфичного для задачи.
...
Задача может иметь несколько пользовательских стилей с разными родительскими стилями. Например, рассмотрим задачу с пользовательским стилем, у которого родительский стиль "Critical" и другим стилем с родительским стилем "Normal". Проще говоря, если задача критична, применяется первый стиль. Если задача становится некритичной, применяется второй стиль (эта логика унаследована от Microsoft Project Professional).
