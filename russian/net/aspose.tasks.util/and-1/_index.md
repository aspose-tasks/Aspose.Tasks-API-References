---
title: Class AndT
second_title: Справочник по Aspose.Tasks для .NET API
description: Aspose.Tasks.Util.And1T сорт. Применяет логическое И к указанным условиям.
type: docs
weight: 2350
url: /ru/net/aspose.tasks.util/and-1/
---
## And&lt;T&gt; class

Применяет логическое И к указанным условиям.

```csharp
public class And<T> : ICondition<T>
```

| Параметр | Описание |
| --- | --- |
| T | Тип объекта, к которому применяется интерфейс метода. |

## Конструкторы

| Имя | Описание |
| --- | --- |
| [And](and/)(ICondition&lt;T&gt;, ICondition&lt;T&gt;) | Инициализирует новый экземпляр`And` класс. |

## Методы

| Имя | Описание |
| --- | --- |
| [Check](../../aspose.tasks.util/and-1/check/)(T) | Возвращает true, если указанный объект удовлетворяет условиям. |

### Смотрите также

* interface [ICondition&lt;T&gt;](../icondition-1/)
* пространство имен [Aspose.Tasks.Util](../../aspose.tasks.util/)
* сборка [Aspose.Tasks](../../)


