---
title: Prj.FiscalYearStart
second_title: Aspose.Tasks for .NET API Referansı
description: Prj alan. Mali yıl numaralandırmasının kullanılıp kullanılmadığını belirler.
type: docs
weight: 330
url: /tr/net/aspose.tasks/prj/fiscalyearstart/
---
## Prj.FiscalYearStart field

Mali yıl numaralandırmasının kullanılıp kullanılmadığını belirler.

```csharp
public static readonly Key<NullableBool, PrjKey> FiscalYearStart;
```

### Ayrıca bakınız

* struct [Key&lt;T,K&gt;](../../key-2/)
* struct [NullableBool](../../nullablebool/)
* enum [PrjKey](../../prjkey/)
* class [Prj](../)
* ad alanı [Aspose.Tasks](../../prj/)
* toplantı [Aspose.Tasks](../../../)


