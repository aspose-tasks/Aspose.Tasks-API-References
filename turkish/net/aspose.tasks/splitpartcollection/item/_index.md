---
title: Item
second_title: Aspose.Tasks for .NET API Referansı
description: Verilen dizinde bir görevin bölünmüş bölümünü alır.
type: docs
weight: 20
url: /tr/net/aspose.tasks/splitpartcollection/item/
---
## SplitPartCollection indexer

Verilen dizinde bir görevin bölünmüş bölümünü alır.

```csharp
public SplitPart this[int index] { get; set; }
```

| Parametre | Tanım |
| --- | --- |
| index | Parça indeksi. |

### Geri dönüş değeri

bölünmüş bir parça.

### Notlar

Endeks sıfır tabanlıdır. Dizin, dizinin sınırlarının dışındaysa null döndürür.

### Ayrıca bakınız

* class [SplitPart](../../splitpart)
* class [SplitPartCollection](../../splitpartcollection)
* ad alanı [Aspose.Tasks](../../splitpartcollection)
* toplantı [Aspose.Tasks](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Tasks.dll -->
