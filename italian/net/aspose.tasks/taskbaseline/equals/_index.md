---
title: Equals
second_title: Riferimento all'API di Aspose.Tasks per .NET
description: Restituisce un valore che indica se questa istanza è uguale alloggetto TaskBaseline specificato.
type: docs
weight: 110
url: /it/net/aspose.tasks/taskbaseline/equals/
---
## Equals(TaskBaseline) {#equals_1}

Restituisce un valore che indica se questa istanza è uguale all'oggetto TaskBaseline specificato.

```csharp
public bool Equals(TaskBaseline other)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| other | TaskBaseline | l'oggetto AssignmentBaseline specificato da confrontare con questa istanza. |

### Valore di ritorno

restituisce true se questa istanza è uguale all'oggetto TaskBaseline specificato; altrimenti falso.

### Guarda anche

* class [TaskBaseline](../../taskbaseline)
* spazio dei nomi [Aspose.Tasks](../../taskbaseline)
* assemblea [Aspose.Tasks](../../../)

---

## Equals(object) {#equals_2}

Restituisce un valore che indica se questa istanza è uguale a un oggetto specificato.

```csharp
public override bool Equals(object obj)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| obj | Object | L'oggetto da confrontare con questa istanza. |

### Valore di ritorno

**Vero** se l'oggetto specificato è una TaskBaseline che ha lo stesso valore UID di questa istanza; altrimenti, **falso**.

### Guarda anche

* class [TaskBaseline](../../taskbaseline)
* spazio dei nomi [Aspose.Tasks](../../taskbaseline)
* assemblea [Aspose.Tasks](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Tasks.dll -->
