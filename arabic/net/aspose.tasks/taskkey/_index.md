...
يمثل قائمة بحقول المهام المدعومة.
...
| Duration | `14` | يمثل حقل المدة (المهمة). |
| DurationVariance | `15` | يمثل حقل DurationVariance (مهمة). |
| DurationFormat | `16` | يمثل حقل DurationFormat (مهمة). |
| Work | `17` | يمثل حقل العمل (المهمة) . |
| Stop | `18` | يمثل حقل الإيقاف (المهمة) . |
| Resume | `19` | يمثل حقل استئناف (مهمة). |
| IsResumeValid | `20` | يمثل الحقل IsResumeValid (مهمة). |
| IsEffortDriven | `21` | يمثل الحقل IsEffortDriven (مهمة). |
| IsRecurring | `22` | يمثل الحقل IsRecurring (مهمة). |
| IsOverallocated | `23` | يمثل الحقل غير المفرط (مهمة). |
| HasOverallocatedResource | `24` | يمثل الحقل HasOverallocatedResource (المهمة). |
| IsEstimated | `25` | يمثل الحقل التقديري (المهمة). |
| IsMilestone | `26` | يمثل حقل IsMilestone (مهمة). |
| IsCritical | `27` | يمثل الحقل IsCritical (مهمة). |
| IsSubproject | `28` | يمثل حقل المشروع الفرعي (مهمة). |
| IsSubprojectReadOnly | `29` | يمثل الحقل IsSubprojectReadOnly (مهمة). |
| IsMarked | `30` | يمثل الحقل IsMarked (مهمة). |
| IgnoreWarnings | `31` | يمثل حقل IgnoreWarnings (مهمة). |
| SubprojectName | `32` | يمثل حقل اسم المشروع الفرعي (المهمة). |
| IsExternalTask | `33` | يمثل الحقل IsExternalTask (المهمة). |
| IsSummary | `34` | يمثل حقل IsSummary (المهمة). |
...
| ActualDuration | `58` | يمثل الحقل ActualDuration (المهمة). |
| ActualCost | `59` | يمثل حقل التكلفة الفعلية (مهمة). |
| ActualOvertimeCost | `60` | يمثل الحقل ActualOvertimeCost (مهمة). |
| ActualWork | `61` | يمثل حقل ActualWork (المهمة). |
| ActualOvertimeWork | `62` | يمثل الحقل ActualOvertimeWork (مهمة). |
| OvertimeWork | `63` | يمثل حقل العمل الإضافي (مهمة). |
| RegularWork | `64` | يمثل حقل العمل العادي (المهمة). |
| RemainingDuration | `65` | يمثل حقل المدة المتبقية (المهمة). |
| RemainingCost | `66` | يمثل حقل التكلفة المتبقية (المهمة). |
...
| Deadline | `76` | يمثل حقل الموعد النهائي (المهمة). |
...
| HyperlinkAddress | `84` | يمثل حقل عنوان الارتباط التشعبي (مهمة). |
| HyperlinkSubAddress | `85` | يمثل الحقل HyperlinkSubAddress (مهمة). |
| IgnoreResourceCalendar | `86` | يمثل الحقل IgnoreResourceCalendar (مهمة). |
| HideBar | `87` | يمثل حقل HideBar (مهمة). |
| IsRollup | `88` | يمثل حقل IsRollup (مهمة). |
| BCWS | `89` | يمثل حقل BCWS (المهمة). |
| BCWP | `90` | يمثل حقل BCWP (مهمة). |
| PhysicalPercentComplete | `91` | يمثل الحقل PhysicalPercentComplete (مهمة). |
| EarnedValueMethod | `92` | يمثل الحقل EarnedValueMethod (مهمة). |
| ActualWorkProtected | `93` | يمثل الحقل ActualWorkProtected (مهمة). |
| ActualOvertimeWorkProtected | `94` | يمثل الحقل ActualOvertimeWorkProtected (مهمة). |
| IsPublished | `95` | يمثل الحقل IsPublished (مهمة). |
| IsScheduled | `96` | يمثل الحقل المجدول (المهمة). |
| StatusManager | `97` | يمثل حقل StatusManager (المهمة). |
...
| DisplayOnTimeline | `119` | يمثل حقل DisplayOnTimeline (مهمة). |
| TaskIsAssigned | `120` | يمثل حقل TaskIsAssigned (مهمة). |
| TaskOriginalStart | `121` | يمثل حقل البداية (المهمة) الأصلي. |
| TaskOriginalFinish | `122` | يمثل حقل النهاية الأصلي (المهمة). |
| IsShowBeforeProjectStartDateWarning | `123` | يمثل العلامة التي تجعل MSP تظهر تحذيرًا بأن المهمة تبدأ قبل تاريخ بدء المشروع. |
