---
title: Equals
second_title: Aspose.Tasks لمرجع .NET API
description: إرجاع إشارة تشير إلى ما إذا كان هذا المثيل مساوياً لمثيلNullableBoolaspose.tasks/nullablebool فئة .
type: docs
weight: 40
url: /ar/net/aspose.tasks/nullablebool/equals/
---
## Equals(NullableBool) {#equals}

إرجاع إشارة تشير إلى ما إذا كان هذا المثيل مساوياً لمثيل[`NullableBool`](../../nullablebool) فئة .

```csharp
public bool Equals(NullableBool other)
```

| معامل | يكتب | وصف |
| --- | --- | --- |
| other | NullableBool | الكائن المحدد للمقارنة مع هذا المثال. |

### قيمة الإرجاع

علامة تشير إلى ما إذا كان هذا المثيل مساويًا للمثيل المحدد لملف[`NullableBool`](../../nullablebool) صف دراسي.

### أنظر أيضا

* struct [NullableBool](../../nullablebool)
* مساحة الاسم [Aspose.Tasks](../../nullablebool)
* المجسم [Aspose.Tasks](../../../)

---

## Equals(object) {#equals_1}

إرجاع إشارة تشير إلى ما إذا كان هذا المثيل يساوي الكائن المحدد.

```csharp
public override bool Equals(object obj)
```

| معامل | يكتب | وصف |
| --- | --- | --- |
| obj | Object | الكائن المحدد للمقارنة مع هذا المثال. |

### قيمة الإرجاع

علامة تشير إلى ما إذا كان هذا المثيل يساوي الكائن المحدد.

### أنظر أيضا

* struct [NullableBool](../../nullablebool)
* مساحة الاسم [Aspose.Tasks](../../nullablebool)
* المجسم [Aspose.Tasks](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Tasks.dll -->
