---
title: "ListUtils.Find"
second_title: "مرجع API لـ Aspose.Tasks لـ .NET"
description: "طريقة ListUtils. العثور على أول ظهور لعنصر قائمة يحقق الشرط المحدد"
type: docs
weight: 30
url: /ar/net/aspose.tasks.util/listutils/find/
---
## ListUtils.Find&lt;T&gt; method

العثور على أول ظهور لعنصر في القائمة يحقق الشرط المحدد.

```csharp
public static T Find<T>(IList<T> list, ICondition<T> cond)
```

| معامل | الوصف |
| --- | --- |
| T | نوع الكائن للبحث عنه. |
| قائمة | قائمة للمعالجة. |
| شرط | الشرط المستخدم للعثور على عنصر في القائمة المحددة. |

### قيمة الإرجاع

عنصر القائمة أو null.

## الأمثلة

يعرض كيفية العمل مع طريقة Find في أداة القوائم.

```csharp
public void WorkWithListUtilsFind()
{
    var project = new Project(DataDir + "Project2003.mpp");
    List<Filter> taskFilters = project.TaskFilters.ToList();

    Assert.AreEqual(3, taskFilters.Count, "Project.TaskFilters count");

    var filter = ListUtils.Find(taskFilters, new FilterByName("&All Tasks"));

    Console.WriteLine("Name: " + filter.Name);
    Console.WriteLine("Filter Type: " + filter.FilterType);
    Console.WriteLine("Show In Menu: " + filter.ShowInMenu);
    Console.WriteLine("Show Related Summary Rows: " + filter.ShowRelatedSummaryRows);
}

public class FilterByName : ICondition<Filter>
{
    private readonly string name;

    public FilterByName(string name)
    {
        this.name = name;
    }

    /// <summary>
    /// تُعيد true إذا كان الكائن المحدد يفي بالشروط.
    /// </summary>
    /// <param name=\"el\">الكائن للتحقق منه.</param>
    /// <returns>True إذا كان الكائن يفي بالشروط.</returns>
    /// <inheritdoc />
    public bool Check(Filter el)
    {
        return el.Name == this.name;
    }
}
```

### انظر أيضًا

* interface [ICondition&lt;T&gt;](../../icondition-1/)
* class [ListUtils](../)
* namespace [Aspose.Tasks.Util](../../listutils/)
* assembly [Aspose.Tasks](../../../)


